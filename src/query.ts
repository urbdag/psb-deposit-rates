...
export const TENURE_PRESETS: { label: string; days: number }[] = [
...
export const AMOUNT_PRESETS: { label: string; amount: number }[] = [
...
export function tenureMatches(range: TenureRange, days: number): boolean {
...
export function applicableEntries(
  ...
  return rates
    ...
    .filter((e) => {
      if (query.product === "SAVINGS") return true;
      if (query.tenureDays == null) return true;
...
export function bestEntryForBank(
  rates: RateEntry[],
  bankId: string,
  query: RateQuery,
): RateEntry | undefined {
 ...
 */
export function rankBanks(dataset: Dataset, query: RateQuery): RankedRate[] {
  const bankById = new Map<string, Bank>(dataset.banks.map((b) => [b.id, b]));
  const rows: { bank: Bank; entry: RateEntry }[] = [];
  ...
  for (const bank of banks) {
    ...
    if (best) rows.push({ bank, entry: best });
  }
 ...
 */
export function bestByTenure(
  dataset: Dataset,
  base: Omit<RateQuery, "tenureDays">,
): { tenureLabel: string; days: number; top?: RankedRate }[] {
...
export function headlineRate(
  dataset: Dataset,
  product: RateQuery["product"],
  customer: RateQuery["customer"],
  category?: RateQuery["category"],
): RankedRate | undefined {
  const bankById = new Map(dataset.banks.map((b) => [b.id, b]));
  const candidates = dataset.rates.filter((e) => {
    if (e.product !== product || e.customer !== customer) return false;
    if (category) {
      const bank = bankById.get(e.bankId);
      if (!bank || bank.category !== category) return false;
    }
    return true;
  });
  if (candidates.length === 0) return undefined;
  let best: RateEntry = candidates[0];
  for (const e of candidates) if (e.ratePercent > best.ratePercent) best = e;
