...
import type { TableAdapterConfig } from "./base.js";
...
/**
 * Config for {@link PrivateTableAdapter}. Extends the base config with the
 * column indices of the retail general / senior rate columns, so multi-column
 * private tables (amount-slab grids) map the correct columns.
 */
export interface PrivateTableConfig extends TableAdapterConfig {
  /**
   * Zero-based cell index of the retail (< ₹3 crore) GENERAL rate column.
   * Default 1 (the first column after the tenure label), which is correct for
   * a plain (tenure, general, senior) table.
   */
  generalCol?: number;
  /**
   * Zero-based cell index of the retail (< ₹3 crore) SENIOR-citizen rate
   * column. Default 2. For a 4-column "General <3cr | General 3-5cr | Senior
   * <3cr | Senior 3-5cr" grid this is 3 (skipping the 3-5cr general column).
   */
  seniorCol?: number;
}
...
export class PrivateTableAdapter extends TableRateAdapter {
  protected readonly generalCol: number;
  protected readonly seniorCol: number;

  constructor(config: PrivateTableConfig) {
    super(config);
    this.generalCol = config.generalCol ?? 1;
    this.seniorCol = config.seniorCol ?? 2;
  }

  override parseFdRd(html: string, effectiveDate: string, url?: string): RateEntry[] {
    const fdUrl =
      url ??
      (Array.isArray(this.cfg.fdUrl) ? this.cfg.fdUrl[0] : this.cfg.fdUrl);
    const source: RateSource = { url: fdUrl, effectiveDate, quality: "OFFICIAL" };
    ...
    for (const cells of table) {
      ...
      const senior =
        cells.length > this.seniorCol ? parsePercent(cells[this.seniorCol]) : null;
   ...
   */
  protected findPrivateRateTable(html: string): string[][] | null {
    for (const table of extractTables(html)) {
      const rows = extractRows(table);
      const dataRows: string[][] = [];
      for (const cells of rows) {
        if (cells.length <= this.generalCol) continue;
