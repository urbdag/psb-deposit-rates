...
const CAPITAL_FD_URL =
  "https://www.capital.bank.in/interest-rates/callable-domestic-term-deposit";
...
export class CapitalsfbAdapter extends TableRateAdapter {
  ...
  override parseFdRd(
    ...
    const seniorByTenure = new Map<string, number>();
...
function tenureKey(t: { minDays: number; maxDays: number | null }): string {
 ...
 */
export function findCapitalRateTables(html: string): [string, number][][] {
  ...
  for (const table of extractTables(html)) {
    ...
    if (pairs.length >= 4 && distinct.size >= 4) out.push(pairs);
  }
  return out;
}
