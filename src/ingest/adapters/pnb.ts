...
export class PnbAdapter extends TableRateAdapter {
  ...
  override parseFdRd(
    ...
    if (!table) return super.parseFdRd(html, effectiveDate, url);
 ...
 */
function findPnbDomesticTable(html: string): string[][] | null {
  ...
  for (const table of extractTables(html)) {
    ...
    if (!/rates?\s+for\s+public/.test(tableText)) continue;
    ...
    if (/\bnre\b/.test(tableText)) continue;
    if (/3\s*cr\b.*(?:10\s*cr|rs\.?\s*10)/.test(tableText)) continue;
    if (/rs\.?\s*3\s*cr.*to.*rs\.?\s*10\s*cr/.test(tableText)) continue;
    ...
    for (const cells of rows) {
      ...
      if (!tenure) continue;
      if (parsePercent(cells[2]) == null) continue; // general RoI required
      dataRows.push(cells);
    }
