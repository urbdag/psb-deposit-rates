...
/**
 * One row of ICICI's client-side `window.interestData` rate matrix.
 * `c1` = retail (< ₹3 crore) GENERAL rate, `c2` = retail (< ₹3 crore) SENIOR
 * rate. `c3` / `c4` are the ₹3cr–₹5cr (bulk) columns and are ignored.
 */
interface IciciRateRow {
  tenure: string;
  c1: number;
  c2: number;
  c3?: number;
...
export class IciciAdapter extends TableRateAdapter {
  ...
  override parseFdRd(
    ...
    for (const row of rows) {
      ...
      if (/tax\s*saver/i.test(row.tenure)) continue;
      ...
      const senior = row.c2;
      if (typeof general !== "number" || Number.isNaN(general)) continue;
      ...
      if (hasSenior) out.push(this.fd("SENIOR", senior, tenure, source, scheme));
      ...
      if (
        ...
        if (hasSenior) out.push(this.rd("SENIOR", senior, tenure, source));
      }
    }
 ...
 */
export function extractIciciRetailRows(html: string): IciciRateRow[] | null {
  const globals = extractRateGlobals(html);
  if (!globals) return null;
  const interestData = globals["interestData"];
  if (!Array.isArray(interestData) || interestData.length === 0) return null;
  const first = interestData[0];
  if (!Array.isArray(first) || first.length === 0) return null;
  const rows: IciciRateRow[] = [];
  for (const r of first) {
    if (
      r &&
      typeof r === "object" &&
      typeof (r as IciciRateRow).tenure === "string" &&
      typeof (r as IciciRateRow).c1 === "number"
    ) {
      rows.push(r as IciciRateRow);
    }
  }
