...
import type { AmountThreshold, RateEntry, RateSource, TenureRange } from "../../types.js";
...
const FINO_PDF_URL =
  "https://www.fino.bank.in/files/fino/media/post_attachments/sites/default/files/2025-11/Savings%20Account%20Revised%20Interest%20Rates%20Effective%20from%201st%20Dec%202025.pdf";
...
export class FinoAdapter extends TableRateAdapter {
  ...
  override parseFdRd(): RateEntry[] {
    return [];
  }

  /** Payments bank: NEVER emit FD/RD from a PDF either. */
  override parsePdfFdRd(): RateEntry[] {
    return [];
  }
   ...
   */
  override async fetchRates(): Promise<RateEntry[]> {
    ...
    throw new Error(
      `${this.bankId}: no SAVINGS rates from PDF rate card:\n    ` +
        attempts.join("\n    "),
    );
  }
  ...
  parseSavingsPdf(text: string, url?: string): RateEntry[] {
    ...
    const anyBalance: AmountThreshold = {
      minAmount: 0,
      maxAmount: null,
      label: "All balances",
    };
    ...
    for (const slab of slabs) {
      for (const customer of ["GENERAL", "SENIOR"] as const) {
        out.push({
          bankId: this.bankId,
          product: "SAVINGS",
          customer,
...
export function parseBalanceBand(line: string): AmountThreshold | null {
  ...
  const hasBalanceWord =
    /balance|bal\.|amount|slab|up\s*to|upto|above|rs\.?|₹|inr|lakh|lac|crore/i.test(
      line,
    );
  if (!hasBalanceWord) return null;
  // Reject tenure lines outright (payments banks have none, but guard anyway).
  if (/\b(day|days|month|months|year|years|yr)\b/i.test(line)) return null;

  const amounts = extractRupeeAmounts(line);
  const hasUpto = /\b(up\s*to|upto|less than|below|maximum|max\b)/i.test(lower);
  const hasAbove = /\b(above|more than|greater than|exceeding|and above|\bmin\b)/i.test(
    lower,
  );
 ...
 * lakh/crore word.
 */
function extractRupeeAmounts(line: string): number[] {
  const out: number[] = [];
  ...
  for (const g of grouped) {
    ...
    if (digits.length < 3) continue; // skip rates like "3" / "25"
