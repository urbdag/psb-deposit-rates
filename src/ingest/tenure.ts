 ...
 */
export function parseTenure(text: string): TenureRange | null {
  const t = text.toLowerCase().replace(/\s+/g, " ").trim();
  if (!/\d/.test(t)) return null;
  if (!/(day|year|yr|month|mon)/.test(t)) return null;
  ...
  if (
    /(premature|penalty|notice|closure|remained|w\.e\.f|prior notice)/.test(t)
  )
    return null;

  const toDays = (value: number, unit: string): number => {
    if (/year|yr/.test(unit)) return Math.round(value * 365);
    if (/month|mon/.test(unit)) return Math.round(value * 30);
  ...
  const shared = t.match(
    /(\d+(?:\.\d+)?)\s*(?:-|–|to)\s*(\d+(?:\.\d+)?)\s*(years?|yrs?|months?|mons?|days?)\b/,
  );
  ...
  const pairs: { value: number; unit: string }[] = [];
