...
const CANARA_TERM_DEPOSIT_URL = "https://www.canarabank.bank.in/term-deposits-rate-of-interest-p.a.";
/**
 * Canara Bank adapter.
 *
 ...
 */
function findCanaraRetailTable(html) {
    for (const table of extractTables(html)) {
        ...
        for (const cells of rows) {
            if (cells.length < 4)
                continue; // need at least tenure + gen RoI + yield + sr RoI
            const tenure = parseTenure(cells[0]);
            if (!tenure)
                continue;
            if (parsePercent(cells[1]) == null)
        ...
        if (dataRows.length >= 3 && distinctTenures.size >= 4)
            return dataRows;
    }
    return null;
}
