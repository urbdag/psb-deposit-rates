...
const CITYUNION_FD_URL = "https://www.cityunionbank.com/deposit-interest-rate";
/**
...
export class CityunionAdapter extends PrivateTableAdapter {
    ...
    findPrivateRateTable(html) {
        ...
        for (const table of extractTables(html)) {
            ...
            const isForeign = /\bnre\b|\bnro\s*foreign|\bfcnr\b|foreign\s*currency/.test(signature);
            const isRetail = /callable\s*term\s*deposit/.test(signature);
            if (isSavings || isForeign)
                continue;
            if (isRetail)
                return dataRows;
            if (fallback == null)
