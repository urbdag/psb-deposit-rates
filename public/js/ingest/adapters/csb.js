 ...
 */
function sectionContextFor(html, table) {
 ...
 */
function findCsbDomesticTermTable(html) {
    let best = null;
    let bestCount = 0;
    for (const table of extractTables(html)) {
        const signature = (sectionContextFor(html, table) +
            " " +
            stripTags(table)).toLowerCase();
        // Reject savings and foreign-currency tables outright.
        if (/savings?\s*bank\s*deposit|domestic\s*savings/.test(signature))
            continue;
        if (/\bnre\b|\bfcnr\b|foreign\s*currency/.test(signature))
            continue;
        // Must be a TERM deposit table (title lives in the heading above the table).
        if (!/term\s*deposit/.test(signature))
            continue;
        ...
        for (const cells of rows) {
            ...
            if (!/^\d+$/.test(cells[0].trim()))
                continue; // leading serial column
            ...
            if (parsePercent(cells[2]) == null)
                continue; // retail general required
            dataRows.push(cells);
        }
