import { bestByTenure, headlineRate, rankBanks } from "./query.js?v=8c4ced7ec2";
import { amountLabel, assessFreshness, formatDate, formatINR, formatRate, parseAmountInput, productLabel, } from "./format.js?v=8c4ced7ec2";
import { banksChangedCount, recentChanges } from "./history.js?v=8c4ced7ec2";
...
function readStateFromUrl() {
    ...
    else if (sector === "payments_bank")
        state.category = "PAYMENTS_BANK";
    else if (sector === "foreign")
...
async function boot() {
    ...
    if (compareParam) {
        ...
        if (dataset.banks.some((x) => x.id === a) &&
...
function renderShell() {
    ...
    const navLink = (href, label, key) => el("a", { class: `nav-link${key === "home" ? " nav-active" : ""}`, href }, [
        label,
    ]);
    root.append(el("nav", { class: "nav", id: "nav" }, [
        ...
        el("div", { class: "nav-drawer", id: "nav-drawer" }, [
            ...
            ...TENURE_LINKS.map((t) => el("a", { class: "nav-link nav-sub", href: t.href }, [t.label])),
        ...
        el("div", { class: "container" }, [
            el("div", { class: "footer-grid" }, [
                el("div", {}, [
                    ...
                    el("p", { class: "muted" }, [
                        "Deposit rates across India's banks. Informational only — always verify on the bank's official website before investing.",
                    ]),
                ]),
                el("div", {}, [
                    el("div", { class: "legend" }, [
...
function navDropdown(label, href, items, twoCol = false) {
    const trigger = el("a", {
        class: "nav-link",
        href,
        "aria-haspopup": "true",
...
function movementsStrip() {
    const changes = recentChanges(rateHistory);
    if (changes.length === 0)
        return ""; // nothing to show until data accrues
    ...
    const parts = [];
    if (ups)
        parts.push(`<span class="mv-up">${upTri} ${ups} raised</span>`);
    if (downs)
        parts.push(`<span class="mv-down">${downTri} ${downs} cut</span>`);
    const a = el("a", {
        class: "movements-strip",
        href: "rate-movements/",
        html: `<span class="mv-badge">Rates on the move</span>` +
...
function renderExplore() {
    ...
    for (const p of products) {
        ...
        pgrid.append(el("a", { class: "explore-card", href: p.href }, [
    ...
    // Tenure chips
    section.append(el("div", { class: "explore-label muted" }, ["Best FD by tenure"]));
    const tchips = el("div", { class: "chips" });
    for (const t of TENURE_LINKS) {
        tchips.append(el("a", { class: "chip", href: t.href }, [t.label]));
    }
    tchips.append(el("a", { class: "chip", href: "senior-citizen-fd-rates/" }, [
        "Senior citizen",
    ]));
...
// ---- Compare two banks ----------------------------------------------------
const compareState = { a: "", b: "", product: "FD" };
function openCompare(a, b) {
    ...
    function draw() {
        ...
        const rowsFor = (id) => dataset.rates.filter((r) => r.bankId === id &&
        ...
        const rateAt = (rows, key) => {
            const r = rows.find((x) => `${x.tenure.minDays}-${x.tenure.maxDays}-${x.scheme || ""}` === key);
        ...
        const body = keys
            ...
            const win = va != null && vb != null ? (va > vb ? "a" : vb > va ? "b" : "") : "";
            return `<tr>
        ...
        card.append(table);
        card.append(el("div", { class: "modal-source" }, [
            el("span", { class: "muted" }, [
