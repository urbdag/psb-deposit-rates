...
function withCategory(base, category) {
    if (category === "SENIOR")
        return round2(base + SENIOR_ADDON);
    if (category === "SUPER_SENIOR")
        return round2(base + SUPER_SENIOR_ADDON);
    return round2(base);
}
function round2(n) {
    return Math.round(n * 100) / 100;
}
const CATEGORIES = ["GENERAL", "SENIOR", "SUPER_SENIOR"];
function buildRates() {
    const entries = [];
    for (const bank of BANKS) {
        ...
        SAVINGS_SLABS.forEach((slab, i) => {
            ...
            for (const category of ["GENERAL", "SENIOR"]) {
                entries.push({
                    bankId: bank.id,
                    product: "SAVINGS",
                    customer: category,
