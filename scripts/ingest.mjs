...
const { runAdapters } = await import(resolve(root, "public/js/ingest/adapter.js"));

// -------------------------------------------------------------------------
// Register real adapters here as they are implemented, e.g.:
//   import { SbiAdapter } from "../public/js/ingest/adapters/sbi.js";
//   const ADAPTERS = [new SbiAdapter(), ...];
// The *.example.ts template intentionally throws, so it is NOT registered.
const ADAPTERS = [];
// -------------------------------------------------------------------------
...
function validateBankRates(bankId, rates) {
  ...
  for (const r of rates) {
    ...
    if (!["FD", "SAVINGS", "RD"].includes(r.product)) errors.push(`bad product: ${r.product}`);
    ...
    if (r.source?.quality !== "OFFICIAL") errors.push("source.quality must be OFFICIAL");
  }
...
async function loadExisting() {
  try {
    ...
    return { banks: BANKS, rates: [], generatedAt: null, containsSampleData: false };
  }
}
...
async function main() {
  ...
  if (ADAPTERS.length === 0) {
    console.log("No adapters registered — nothing to refresh. Dataset left unchanged.");
    console.log("Register per-bank adapters in scripts/ingest.mjs (ADAPTERS) to enable daily refresh.");
    return 0;
  }
  ...
  for (const bank of BANKS) {
    ...
    if (scraped) {
      ...
      console.warn(`✗ ${bank.id}: validation failed (${errs.join("; ")}) — keeping last-known-good`);
  ...
  console.log(`Fell back to last-known-good: ${fellBack.length ? fellBack.join(", ") : "none"}`);
